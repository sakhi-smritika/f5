"""Source-of-Knowledge strategies: fetch raw candidate bits for a query.

The default ``LLMSource`` synthesizes bits with LiteLLM. ``WebSearchSource`` is a
registered stub kept behind the same contract so a real web search can drop in
later without touching the orchestrator or endpoints.
"""
...
from .base import KBCandidate, Query, Registry, SourceStrategy

logger = logging.getLogger(__name__)

SOURCE_STRATEGIES: Registry[SourceStrategy] = Registry("source")
...
_SYSTEM_PROMPT = (
    ...
    "avoid list. Respond with ONLY a JSON array of objects, each with exactly "
...
def _parse_bits(raw: str, limit: int) -> list[KBCandidate]:
    ...
    try:
        ...
        logger.warning("LLMSource returned non-JSON content; skipping")
        return []
...
@SOURCE_STRATEGIES.register("llm", default=True)
class LLMSource:
    """Default source: ask a LiteLLM-backed model to synthesize bits."""

    def search(self, query: Query, limit: int) -> list[KBCandidate]:
        query_text = query.to_text() or "General personal-growth knowledge."
        ...
        response = litellm.completion(
            ...
            messages=[
                {"role": "system", "content": _SYSTEM_PROMPT},
                {
                    "role": "user",
                    "content": (
                        f"Generate {limit} knowledge bits.\n\n{query_text}"
                    ),
                },
            ],
        )
...
@SOURCE_STRATEGIES.register("web_search")
class WebSearchSource:
    """Placeholder for a future web-search-backed source.
    ...
    def search(self, query: Query, limit: int) -> list[KBCandidate]:
        raise NotImplementedError(
            "web_search source is not implemented yet; use 'llm'."
        )
